...
import { mockOther } from '@/lib/mockResponseDataCategories';
  ...
  return (
    <Link href={`/books/${book.id}`} className="block w-[220px] flex-shrink-0 bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all border-[1px] border-green-400">
      <div className="flex flex-col h-full">
        <div className="w-full h-[220px] relative overflow-hidden bg-[#ffb700]">
          <Image 
...
export default function OtherBookSection() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setBooks(mockOther.data as any);
      setIsLoading(false);
    }, 500);
  }, []);
  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-medium">Các sách khác</h2>
        <Link href="/books/other" className="text-[#1ab69d] hover:underline text-sm flex items-center">
          Xem tất cả
